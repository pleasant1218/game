...
const Data = {
  ...
  async loadRemote() {
    try {
      ...
      // Also read localStorage so we can migrate old data to Supabase
      ...
      const merge = (id) => {
        ...
        // Start from defaults, then cloud, then fill gaps from localStorage
        const base = { ...def[id], ...c };
        // If cloud row has no goals/todos but local does — prefer local (migration scenario)
        ...
        // Always migrate legacy localStorage PIN (takes priority during first migration)
      ...
      // Always upload merged state to Supabase (creates row if missing, updates if existing)
