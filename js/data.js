...
const Data = {
  ...
  async loadRemote() {
    try {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/game_state?id=in.(bjerg,hungry)&select=id,data`,
      ...
      this._cache = {
        players: {
          bjerg:  { ...def.bjerg,  ...(map.bjerg  || {}) },
          hungry: { ...def.hungry, ...(map.hungry || {}) },
        },
        sharedMemories: [],
        version: 1,
      };

      // Migrate legacy localStorage PINs into player data
      for (const id of ['bjerg', 'hungry']) {
        const lp = localStorage.getItem(`pin_${id}`);
        if (lp && !this._cache.players[id].pin) {
          this._cache.players[id].pin = lp;
          localStorage.removeItem(`pin_${id}`);
          this._savePlayerRemote(id);
        }
      }

      // Create rows that don't exist yet
      for (const id of ['bjerg', 'hungry']) {
        if (!map[id]) await this._upsertPlayer(id, this._cache.players[id]);
      }
  ...
  async _upsertPlayer(id, data) {
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/game_state`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      console.warn(`Supabase save failed for ${id}:`, e.message);
    }
  },
