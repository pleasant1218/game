...
function buildSpriteGrid(player) {
  ...
  const grid = _baseSpriteGrid(player, { T, sk, skH, hr, hrH, hrD, hrX, st, stH, stD, pn, pnD, blt });
  if (outfit.shape && _OUTFIT_OVERLAYS[outfit.shape]) {
    _OUTFIT_OVERLAYS[outfit.shape](grid, { T, sk, st, stH, stD });
  }
  return grid;
}

// ─── Outfit overlays (post-process the base grid for shop variants) ──────────
...
const _OUTFIT_OVERLAYS = {
  ...
  hoodie(grid, { T, st, stH, stD }) {
    grid[13] = grid[13].slice();
    for (let col = 5; col <= 8; col++) grid[13][col] = stD;

    grid[14] = grid[14].slice();
    grid[14][4] = stD;   grid[14][9] = stD;
  ...
  // the collar, plus two button dots.
  jacket(grid, { st, stH, stD }) {
    grid[14] = grid[14].slice();
    grid[14][5] = stH;   grid[14][8] = stH;
    grid[14][6] = '#FFFFFF';
    grid[14][7] = '#FFFFFF';
...
function _baseSpriteGrid(player, c) {
  const { T, sk, skH, hr, hrH, hrD, hrX, st, stH, stD, pn, pnD, blt } = c;
